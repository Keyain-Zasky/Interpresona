...
def test_mask_control_codes_roundtrip():
    ...
    result = mask(raw)
    assert "⟪VAR_0⟫" in result.text
    assert "⟪VAR_1⟫" in result.text
    assert "⟪VAR_2⟫" in result.text
...
def test_mask_placeholder_at_start():
    ...
    result = mask(raw)
    assert result.text.startswith("⟪VAR_0⟫")
    assert unmask(result.text, result.placeholders) == raw
...
def test_mask_placeholder_at_end():
    ...
    result = mask(raw)
    assert result.text.endswith("⟪VAR_0⟫")
    assert unmask(result.text, result.placeholders) == raw
...
def test_validate_placeholders_extra():
    ...
    errors = validate_placeholders("Hello ⟪VAR_0⟫", result.placeholders)
    assert len(errors) == 1
...
def test_pipeline_control_codes_masked_in_records():
    ...
    assert "⟪VAR_" in rec0.masked_text
...
def test_mock_translator_preserves_placeholders():
    ...
    results = mt.translate(["Hello ⟪VAR_0⟫ world ⟪VAR_1⟫!"])
    assert "⟪VAR_0⟫" in results[0]
    assert "⟪VAR_1⟫" in results[0]
